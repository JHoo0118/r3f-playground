  ...
  const particles = useMemo(
    () =>
      Array.from({ length: nbParticles }, (_, __) => ({
        position: [randFloat(25, 30), randFloatSpread(50), 0],
...
const Particle = ({ position, size, rotation, lifetime }: ParticleProps) => {
  ...
  useFrame(({ camera }, delta) => {
    ...
    if (ref.current.material) {
      (ref.current.material as MeshBasicMaterial).color.r = lerp(
        colorStart.r,
        colorEnd.r,
        lifetimeProgression
      );
      (ref.current.material as MeshBasicMaterial).color.g = lerp(
        colorStart.g,
        colorEnd.g,
        lifetimeProgression
      );
      (ref.current.material as MeshBasicMaterial).color.b = lerp(
        colorStart.b,
        colorEnd.b,
        lifetimeProgression
      );
