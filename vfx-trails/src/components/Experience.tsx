...
import { AdditiveBlending, DirectionalLight, Mesh } from "three";
...
export const Experience = () => {
  ...
  return (
    <>
      ...
      <Lights sun={sun as unknown as React.RefObject<Mesh>} />
...
interface LightsProps {
  sun: React.RefObject<Mesh> | null;
}

const Lights = ({ sun }: LightsProps) => {
  ...
  useFrame(() => {
    ...
    if (sun) {
      (sun as unknown as Mesh).position.y = lerp(
        -80,
        -72,
        data.range(0, 1 / 4)
      );
      (sun as unknown as Mesh).position.x = lerp(
        0,
        5,
        data.range(1 / 4, 1 / 4)
      );
      (sun as unknown as Mesh).position.z = lerp(
        -100,
        -100,
        data.range(2 / 4, 1 / 4)
      );
