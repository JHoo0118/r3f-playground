 ...
 */
export const VFXEmitter = forwardRef<Object3D, VFXEmitterProps>(
  (
    { debug, emitter, settings = {}, ...props }: VFXEmitterProps,
    forwardedRef
  ) => {
    const [
      {
    ...
    const emit = useVFX((state: any) => state.emit);
    ...
    const settingsBuilder = useMemo(
      ...
      [debug, settings, setSettings, onRestart]
    );
