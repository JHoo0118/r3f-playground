...
export const Experience = ({ section }: ExperienceProps) => {
  ...
  const playTransition = () => {
    if (window.innerWidth > SMALL_SCREEN_THRESHOLD) {
      controls.current?.setLookAt(
        ...(cameraPositions[
          sections[section] as keyof typeof cameraPositions
        ] as [number, number, number, number, number, number]),
        true
      );
    } else {
      controls.current?.setLookAt(
        ...(cameraPositionsSmallScreen[
          sections[section] as keyof typeof cameraPositionsSmallScreen
        ] as [number, number, number, number, number, number]),
        true
      );
    }
