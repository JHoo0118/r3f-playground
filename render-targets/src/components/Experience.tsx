...
export const Experience = () => {
  ...
  useFrame(({ camera, gl, scene }) => {
    ...
    if (tvMaterial.current) {
      ...
      let currentScreenTexture = videoTexture as Texture;
      ...
      if (mode === "front") {
        ...
        scene.traverse((node) => {
          if (node instanceof SkinnedMesh && node.morphTargetInfluences) {
      ...
      scene.traverse((node) => {
        if (node instanceof SkinnedMesh && node.morphTargetInfluences) {
          node.morphTargetInfluences[
            (node.morphTargetDictionary as Record<string, number>)["mouthSmile"]
          ] = 0;
          node.morphTargetInfluences[
            (node.morphTargetDictionary as Record<string, number>)["mouthOpen"]
          ] = 0;
        }
      });
    }
