...
export type ScreenType = keyof typeof transitions | "Intro";
export type TargetScreenType = keyof typeof transitions;
...
function App() {
  ...
  const [targetScreen, setTargetScreen] = useState<TargetScreenType>("Home");
  ...
  useEffect(() => {
    project.ready.then(() => {
      ...
      const transition =
        transitions[
          reverse ? currentScreen : (targetScreen as keyof typeof transitions)
        ];
      if (!transition) {
        return;
      }
      ...
      mainSheet.sequence
        .play({
          range: [transition[0], transition[1]],
